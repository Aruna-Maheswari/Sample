...
public class AddressService {
	...
	public Map<String,Object> addingAddress(AddressDetails addressdetails)
	{
	  int usrid=addressdetails.getUsrid();
	...
	try
	{
		String query="INSERT INTO addressdetails(usrid,address,state,pincode)VALUES(:userId,:useraddress,:userstate,:userpincode)";
		SqlParameterSource sqlparametersource = new MapSqlParameterSource().addValue("userId",usrid).addValue("useraddress",address).addValue("userstate",state).addValue("userpincode",pincode);
	...
	return response;
	 ...
	 public Map<String,Object> updateAddress(AddressDetails addressdetails)
	  {
	       Map<String,Object> response=new HashMap<>();
	       try
	       {
	 ...
	 public Map<String, Object> retrieveAddress() {
			...
			try {
				...
				String query = "SELECT usrid,address,state,pincode,date FROM addressdetails ";

				List<Map<String, Object>> addressList = jdbcTemplate.queryForList(query);

				for (Map<String, Object> map : addressList) {

					int usrid = (Integer) map.get("userid");
					...
					finalList.add(Map);
					System.out.println(usrid + " " + address + " " + state + " " + pincode + " " + date);
